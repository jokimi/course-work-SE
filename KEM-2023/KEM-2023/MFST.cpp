...
namespace MFST {
	...
	Mfst::RC_STEP Mfst::step() {								// шаг автомата
		...
		if (lenta_position < lenta_size) {						// если лента не закончилась
			if (ISNS(st.top()))	{								// если на вершине стека нетерминал
				GRB::Rule rule;
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)	{
					// если найдено правило по нетерминалу на вершине стека, заполняем номер текущего правила
					GRB::Rule::Chain chain;
					if ((nrulechain = rule.getNextChain(lenta[lenta_position], chain, nrulechain + 1)) >= 0) {
						// получаем следующую цепочку по терминалу из ленты
					...
					rc = NS_ERROR;			// неизвестный нетерминал
			}
	...
	bool Mfst::start() {
		...
		switch (rc_step) {
			case LENTA_END:
				...
				break;
			case NS_NORULE:
				MFST_TRACE4("------>NS_NORULE")	// не найдено правило грамматики (ошибка в грамматике)
				cout << "--------------------------------------------------------------------------" << endl;
				cout << getDiagnosis(0, buf) << endl;
				break;
			case NS_NORULECHAIN:
				MFST_TRACE4("------>NS_NORULENORULECHAIN") // не найдена подходящая цепочка правила (ошибка в исходном коде)
				break;
			case NS_ERROR:
				MFST_TRACE4("------>NS_ERROR") // неизвестный нетерминальный символ грамматики
				break;
			case SURPRISE:
				MFST_TRACE4("------>SURPRISE") // неожиданный код возврата (ошибка в step)
				break;
		};
		return rc;
	};
	...
	char* Mfst::getDiagnosis(short n, char* buf) {
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0) {
			...
			Error::ERROR err = Error::geterrorin(errid, lex.LexTable.table[lpos].sn, lex.LexTable.table[lpos].posWord);
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "Ошибка %d: %s (строка %d, позиция %d)", err.id, err.message, lex.LexTable.table[lpos].sn, lex.LexTable.table[lpos].posWord);
			rc = buf;
		}
		return rc;
	};
